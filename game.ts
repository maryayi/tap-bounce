...
(function () {
  ...
  /**
   * An obstacle. Vertical extents are derived from `groundY` at draw/collision
   * time (never stored absolutely) so the game stays correct across resizes.
   *  - ground: sits on the track, rect spans [groundY - h, groundY].
   *  - air:    floats, rect spans [bottom - h, bottom] where bottom = groundY - gap.
   */
  interface Obstacle {
    x: number;
    w: number;
    h: number;
    gap: number; // clearance under the block (0 for ground obstacles)
    air: boolean;
    passed: boolean;
  }
  ...
  function toggleMute(): void {
    muted = !muted;
    saveMuted(muted);
    if (!muted) ensureAudio(); // unlock audio on the same gesture
  }
  ...
  function onTap(x: number, y: number): void {
    ...
    switch (state) {
      ...
      case State.Playing:
        ...
        break;
      case State.Over:
        if (overTimer >= RESTART_LOCKOUT) startGame();
        break;
    }
  }
  ...
  function update(dt: number): void {
    ...
    updateParticles(dt);
    if (shake > 0) shake = Math.max(0, shake - shake * dt * 8 - 6 * S * dt);
  ...
  function render(): void {
    ...
    drawObstacles();
    drawTrail();
    if (state !== State.Over || overTimer < 0.05) drawBall();
    drawParticles();
  ...
  function drawTeeth(x: number, bottom: number, w: number): void {
  ...
  function drawHUD(): void {
    ...
    if (state === State.Playing) {
      ...
      if (best > 0) subtitle('Best  ' + best, W / 2, H * 0.34 + 150 * S);
  ...
  function title(text: string, x: number, y: number): void {
    ctx.fillStyle = '#ffffff';
    ctx.font = `800 ${Math.min(64 * S, W * 0.13)}px system-ui, sans-serif`;
    ctx.fillText(text, x, y);
  }
  function subtitle(text: string, x: number, y: number): void {
    ctx.fillStyle = 'rgba(255,255,255,0.92)';
    ctx.font = `600 ${28 * S}px system-ui, sans-serif`;
    ctx.fillText(text, x, y);
  }
  function hint(text: string, x: number, y: number): void {
  ...
  function roundRectTop(x: number, y: number, w: number, h: number, r: number): void {
    ctx.beginPath();
    ctx.moveTo(x + r, y);
    ctx.arcTo(x + w, y, x + w, y + h, r);
    ctx.lineTo(x + w, y + h);
  ...
  function frame(now: number): void {
    ...
    lastTime = now;
    if (dt > 1 / 20) dt = 1 / 20; // clamp big gaps (tab switch, lag)
